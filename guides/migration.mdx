...
client = OpenAI(
    api_key="sg-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",  # SaveGate API key
...
```bash After
SAVEGATE_API_KEY=sg-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
...
llm = ChatOpenAI(
    model="gpt-5.1",
    openai_api_key="sg-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
    openai_api_base="https://api.savegate.ai/v1"
)
```
...
os.environ["OPENAI_API_KEY"] = "sg-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"
...
<AccordionGroup>
  <Accordion title="401 Unauthorized" icon="key">
    ...
    **Solution:**
    ...
    - Check that you're using `sg-` prefix
